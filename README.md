...
2. Details mode is a form dynamically created from the attributes. Including all primary record
...
the server, each record set is either read/write or read only. Any modifiable adorment records
...
The source must be RESTful (i.e. has no state and uses resource based URLs). The data source must
support CRUD operations. The mapping of HTTP method to operation is:

* Use POST for Create
* Use GET for Read (single) and Query (list)
* Use PUT for Update
* Use DELETE for Delete

Optionally the Read and Query operations may also support server side paging, sorting, filtering and
...
are supported. The client side controls are good for record sets upto about 5,000 maximum. At this
...
is first. A descending sort is used if the first character in the field name is '-', e.g.
...
* <b>...</b> All remaining parameters are treated as search filterss supplied as a list of
field names and regular expressions.  The named fields are matched against the regular
expression, e.g.
...
The actual number returned in this header is a matter of preference, but it can logically
...
Similar to constraint 2 above, an addition to the Attribute Editor could be made to allow the unique
...
all adornments are queried in the summary controller (the number of queries  would be
